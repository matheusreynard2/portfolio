...
public class ProdutoEndpointsTest extends TestCase {
	...
	public void adicionarProdutoTest() {
		...
		Produto produtoRetornadoRepository = new Produto();
		
		when(produtoRepository.save(produtoTeste)).thenReturn(produtoRetornadoRepository);
		
		Produto produtoRetornadoService = produtoService.adicionarProduto(produtoTeste);

		assertEquals(produtoRetornadoService, produtoRetornadoRepository);
		
		if (produtoRetornadoService.equals(produtoRetornadoRepository))
	...
	public void calcularValorDescontoTrueTest() {
		...
		if (valorResultadoEsperado == valorRetornado)
			...
			System.out.println("TESTE 2: NÃO Passou. Resultados diferentes.");
